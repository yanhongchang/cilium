...
func (gwc *gatewayConfig) deriveFromPolicyGatewayConfig(gc *policyGatewayConfig) error {
	...
	default:
		// If the gateway config doesn't specify any egress IP or interface, us
...
func ParsePolicy(cenp *v2alpha1.CiliumEgressNATPolicy) (*PolicyConfig, error) {
	...
	switch {
	case egressIP != nil && egressGateway != nil:
		return nil, fmt.Errorf("CiliumEgressNATPolicy cannot have both EgressSourceIP and EgressGateway set")
	case egressIP == nil && egressGateway == nil:
		return nil, fmt.Errorf("CiliumEgressNATPolicy needs either EgressSourceIP or EgressGateway set")
	}
	...
	if egressGateway != nil {
		if egressGateway.Interface != "" && egressGateway.EgressIP != "" {
			return nil, fmt.Errorf("CiliumsEgressNATPolicy's gateway configuration can't specify both an interface and an egress IP")
		}
